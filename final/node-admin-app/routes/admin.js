...
//동적 SQL쿼리를 위한 sequelize 객체 참조하기
var sequelize = db.sequelize;
const { QueryTypes } = sequelize;

//관리자 로그인 여부 체크 미들웨어 함수 참조하기
const { isLogined } = require("./sessionmiddleware.js");
...
*/
router.get("/list", isLogined, async (req, res, next) => {
  ...
  //모든 데이터 목록에서 원하는 컬럼 목록만 조회하기
  // const admins = await db.Admin.findAll({
  //     attributes:['admin_member_id','company_code','admin_id','admin_name','email','telephone','dept_name','used_yn_code','reg_date'],
  //     where:{used_yn_code:'1'},
  //     order: [['reg_date','DESC']]
  // });

  //순수 SQL구문을 DB서버에 전달해서 동일한 결과를 반환받는 방법 (예시)
  let query = `SELECT 
        admin_member_id,
        admin_id,admin_name,
        email,
  ...
  const adminCnt = await db.Admin.count();
  console.log("전체 관리자 계정수:", adminCnt);
...
*/
router.post("/list", isLogined, async (req, res) => {
  //Step1:조회 옵션정보 추출하기
  ...
  //사용자가 입력한 조회옵션에 따라 동적 SQL쿼리를 생성하기
  //회사코드 추가 필터 조건 반영
  if (company_code != 9) {
...
*/
router.get("/create", isLogined, async (req, res, next) => {
...
*/
router.post("/modify", isLogined, async (req, res, next) => {
  ...
  const admin = {
    ...
    used_yn_code: use_yn_code,
    edit_date: Date.now(),
    edit_member_id: 1, //회원 인증 구현 아직
  };

  //db서버에 해당 관리자 계정 정보를 수정하고 실제 수정된 건수가 반환됨
  //update()==> UPDATE admin SET ... WHERE admin_member_id=...
...
*/
router.get("/delete", isLogined, async (req, res, next) => {
  //Step 1 : 관리자 고유번호 추출
  const admin_member_id = req.query.id;

  //Step 2 : 관라지 고유번호에 해당하는 관리자 계정 삭제
...
*/
router.get("/modify/:id", isLogined, async (req, res, next) => {
