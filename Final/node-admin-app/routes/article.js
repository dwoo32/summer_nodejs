...
router.post("/create", upload.single("file"), async (req, res) => {
  ...
  //html 폼에서 file태그에 파일을 첨부하면 req.file이라는 속성으로
  //서버에 업로드된 파일정보를 추출할 수 있다.
  ...
  //Step4: 신규 등록된 게시글의 고유번호를 기반으로 첨부파일 등록 처리
  //업로드한 파일이 있는 경우에만 등록처리
  if (uploadFile) {
    //실제 서버에 업로드 된 파일경로
    const filePath = `/upload/${uploadFile.filename}`;
    const fileName = uploadFile.originalname; //서버에 업로드된 파일명(3224211_a.png)
    const originalFileName = uploadFile.originalname; //업로드한 원본파일명(a.png)
    const fileSize = uploadFile.size; //파일크기(바이트)
    const mimeType = uploadFile.mimetype; //파일의 MIME타입(image/png)
